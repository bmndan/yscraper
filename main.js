...
(function () {
  try {
    // ===== TOKEN (loaded from GitHub token.txt) =====
        ...
        GM_xmlhttpRequest({
          ...
          onload: function (r) {
            ...
            var token = String(r.responseText || "")
              ...
              .trim();
            if (!token) {
              reject(new Error("token.txt is empty"));
              return;
            }
            resolve(token);
          },
    ...
    // Only run on an actual listing page (prevents running on search pages)
    ...
    // ===== CreatedDate -> Unix ms (supports DD/MM/YY + hash-safe selector) =====
    function extractCreatedDateStrict() {
      ...
      function parseDMYToMs(txt) {
        var m = clean(txt).match(/(\d{1,2})[./](\d{1,2})[./](\d{2}|\d{4})/);
        if (!m) return null;

        var d = +m[1], mo = +m[2];
        var y = (m[3].length === 2) ? (2000 + +m[3]) : +m[3];

        // ignore placeholder 00/00/00
        if (d === 0 && mo === 0 && (+m[3] === 0)) return null;

        var dt = new Date(y, mo - 1, d);
        if (dt.getFullYear() !== y || dt.getMonth() !== (mo - 1) || dt.getDate() !== d) return null;

        return dt.getTime(); // IMPORTANT: Memento Date fields reliably accept ms
      }

      // exact node you found: <span class="report-ad_createdAt__HASH">פורסם ב ... 06/08/25</span>
      var el = document.querySelector('span[class^="report-ad_createdAt__"], span[class*="report-ad_createdAt__"]');
      if (el) {
        var ms = parseDMYToMs(el.textContent || "");
        if (ms !== null) return ms;
      }

      // fallback: any element mentioning "פורסם" with a date
      var all = Array.from(document.querySelectorAll("div, span, p"));
      for (var j = 0; j < all.length; j++) {
        var txt = clean(all[j].textContent || "");
        if (!txt.includes("פורסם")) continue;
        var ms2 = parseDMYToMs(txt);
    ...
    function extractContactsUnified() {
      ...
      // Broker/Owner roots (robust "contains" selectors)
      var brokerRoot =
      ...
      // Decide listing type:
      // If ownerRoot exists and there is no agency UI, treat as owner listing.
      ...
      if (!treatAsOwner) {
        ...
        // ===== OWNER LISTING =====
        // Each owner contact box: ad-contacts_adContactInfoBox, inside: ad-contact-info_name + ad-contact-info_phoneNumber
      ...
      // Deduplicate contacts => keep up to 2 unique
      var seen = {};
      var uniq = [];
      ...
      return {
        Agency: Agency, // null for owner listings
    ...
    async function extractAll() {
      ...
      var CreatedDate = extractCreatedDateStrict();
      var contacts = extractContactsUnified();

      if (DEBUG) console.log({ clean: clean, td: td, addr: addr, imgs: imgs, CreatedDate: CreatedDate, contacts: contacts });
